              ...
              <tbody>
                <tr v-for="p in productosFiltrados" :key="p.id"
                  ...
                  <td class="celda-codigo">
                    ...
                    <template v-else>
                      <!-- Código del proveedor (referencia — vive en catalogo_proveedor, no se edita acá) -->
                      <span v-if="p.codigo_proveedor" class="codigo-tag codigo-prov" :title="`Código del proveedor: ${p.codigo_proveedor}`">
                        {{ p.codigo_proveedor }} 🔒
                      </span>
                      <!-- Código interno de Ferre-Util: siempre editable, con o sin código de proveedor -->
                      <span v-if="codigoEditando !== p.id" class="codigo-interno-tag codigo-interno-editable" @click="iniciarEditCodigo(p)" title="Clic para editar código interno">
                        {{ p.codigo || '—' }}
                      </span>
...
.codigo-interno-editable { cursor: pointer; }
.codigo-interno-editable:hover { background: var(--amarillo); color: #1A1A1A; }
